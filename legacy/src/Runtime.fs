...
module Runtime =
    ...
    let rec do_stmt (stmt: Stmt) (ctx: Context) (echo: bool) : Context =
        ...
        | Stmt.Help_ModuleOrCmd id ->
            ...
            | None ->
                if not command then
                    failwith "No such command or module"
    ...
    and eval_expr (ex: Expr) (ctx: Context) : Val =
        ...
        | Expr.StrInterp frags ->
            List.map
                (function
                | StrFrag.Str s -> s
                | StrFrag.Ex x -> (eval_expr x ctx).ToString())
                frags
            |> String.concat ""
            |> Val.Str
            ...
            | Val.Arr xs ->
                ...
                | Val.Num n ->
                    try
                        xs.[int n]
                    with _ ->
                        error sub "Index out of bounds"
                | Val.Str s ->
                    try
                        xs.[Val.Str s |> Coerce.num |> int]
                    with _ ->
                        error sub "Index out of bounds"
        ...
        | Expr.Cond(arms, basecase) ->
            let rec loop arms =
                match arms with
                | (cond, ex) :: arms ->
                    if eval_expr cond ctx |> Coerce.bool then
                        eval_expr ex ctx
                    else
                        loop arms
    ...
    and eval_monop (op: Monop) (ex: Expr) (ctx: Context) : Val =
        match op with
        | Monop.NOT -> eval_expr ex ctx |> Coerce.bool |> not |> Val.Bool
        | Monop.NEG -> eval_expr ex ctx |> Coerce.num |> (fun x -> -x) |> Val.Num
        | Monop.ROUND -> eval_expr ex ctx |> Coerce.num |> System.Math.Round |> Val.Num
        | Monop.LEN ->
            match eval_expr ex ctx with
    ...
    and eval_binop (op: Binop) (left: Expr) (right: Expr) (ctx: Context) : Val =
        ...
        match op with
        | Binop.OR ->
            if eval_expr left ctx |> Coerce.bool then
                Val.Bool true
            else
                eval_expr right ctx |> Coerce.bool |> Val.Bool
        | Binop.AND ->
            if eval_expr left ctx |> Coerce.bool then
                eval_expr right ctx |> Coerce.bool |> Val.Bool
            else
